...
| `MARKET_DATA_API_KEY` | Piyasa verisi sağlayıcı anahtarı | boş (seed veri) |
...
- `FallbackMarketDataProvider`: Seed verisi kullanır (varsayılan)
- Gerçek bir sağlayıcı (örn. Alpha Vantage, Finansal API) eklemek için arayüzü implement edin ve `@Scheduled` sync servisini aktif edin.
